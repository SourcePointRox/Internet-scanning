...
def check() -> list[str]:
    ...
    if not masscan:
        warnings.append("未找到 masscan（bin/masscan.exe）：运行 python scripts/setup_deps.py --masscan 查看安装指引，"
                        "或自动改用 scapy/Npcap 后端。")
    npcap = (Path("C:/Windows/System32/Npcap").is_dir()
             or Path("C:/Windows/System32/Npcap/wpcap.dll").exists())
    if not npcap:
        warnings.append("未检测到 Npcap：运行 python scripts/setup_deps.py --npcap 查看安装指引，"
                        "否则只能以 dry-run 模拟运行。")
    try:
        import scapy  # noqa: F401
    except ImportError:
    ...
    if not (ROOT / "data" / "geoip" / "GeoLite2-City.mmdb").exists():
        warnings.append("未找到 GeoLite2-City.mmdb（可选）：地理/ASN 富化将降级。"
                        "可用 python scripts/setup_deps.py --geoip <KEY> 自动下载。")
    # 配置校验（硬编码检测 / 配额检查 / 目录可写性）
    try:
        from orchestrator.config import load
        warnings.extend(f"[配置] {w}" for w in load().validate())
    except Exception as e:  # noqa: BLE001 —— 配置损坏也要给出明确提示
        warnings.append(f"[配置] 加载失败: {e}")
    return warnings
