...
class ScapyL4Scanner:
    def __init__(self, cfg, out_queue: "queue.Queue[dict]", exclude, bandwidth=None,
                 iface: str | None = None, scan_state=None):
        self.cfg = cfg
        self.out = out_queue
        self.exclude = exclude
        self.bw = bandwidth
        self.scan_state = scan_state          # ScanStateMachine | None（进度持久化）
    ...
    def _find_iface_by_ip(ip: str):
        ...
        try:
            ...
            for i in IFACES.values():
                if i.ip == ip:
                    return i.name
        except Exception as e:  # noqa: BLE001 —— 接口枚举失败：回退自动选卡
            log.debug("按 IP 反查网卡失败: %s", e)
        return None
    ...
    def _run(self, targets: list[str], ports: list[int]) -> None:
        ...
        except Exception as e:  # noqa: BLE001 —— 扫描级异常：上报并落盘状态
            from orchestrator.errors import ErrorReporter
            ErrorReporter.get().report(MODULE, "Scapy 扫描异常", exc=e)
            REGISTRY.set_extra(MODULE, last_error=str(e)[:200])
            if self.scan_state:
                self.scan_state.transition("FAILED", error=str(e)[:200])
        finally:
    ...
    def _flush(self, batch: list, send) -> None:
        ...
        if self.scan_state and self._sent % 5000 < BATCH_SIZE:
            self.scan_state.progress(probes_sent=self._sent)
